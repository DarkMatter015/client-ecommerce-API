...
export function CartProvider({ children }: CartProviderProps ) {
    ...
    const [cartItems, setCartItems] = useState(savedItems ? JSON.parse(savedItems) : []);
    ...
    const cartMetrics = useMemo(() => {
        const totalItems = cartItems.reduce((sum: number, item: IItem) => sum + item.quantity, 0);
    ...
    const addItem = (item: IItem) => {
        const existingItem = cartItems.find((i: IItem) => i.product.id === item.product.id);
        if (existingItem) {
    ...
    const handleUpdateQuantity = (id: number, newQuantity: number) => {
        ...
        setCartItems((prev: IItem[]) =>
            prev.map((item: IItem) =>
                item.id === id ? { ...item, quantity: newQuantity, totalPrice: item.product.price * newQuantity } : item
            )
        );
    };
